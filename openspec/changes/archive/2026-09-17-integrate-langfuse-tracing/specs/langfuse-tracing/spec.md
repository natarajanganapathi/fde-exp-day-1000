...
### Requirement: Score client posts directly to trace without session polling
...
#### Scenario: Score posted with trace ID and structured metadata

- **WHEN** a caller invokes score posting with a valid trace ID, score name, and numeric value
- **THEN** the system POSTs a payload to the Langfuse Scores API containing `traceId`, `environment` set to `"ci-cd-pipeline"`, and a `metadata` object with `participant_id`, `pod_id`, and `event_id`
